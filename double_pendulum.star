...
def main(config):
    ...
    # Check if we should only render the last frame (for static image output)
    last_frame_only = config.bool("last_frame_only", False)

    # Render all frames from the selected simulation
    all_frames = []
    if last_frame_only:
        # Only render the last frame (for static image generation)
        if len(simulation) > 0:
            last_frame_idx = len(simulation) - 1
            if is_api_mode:
                all_frames.append(render_frame_simple(simulation, last_frame_idx, hsv_to_rgb, api_origin_y, api_name, config, layout))
            else:
                all_frames.append(render_frame(config, sim_idx, last_frame_idx, hsv_to_rgb, layout))
    else:
        # Render all frames
...
def render_frame_simple(simulation, frame_idx, hsv_to_rgb, origin_y, sim_name, config, layout):
    ...
    # Black background (unless transparent mode)
    if not config.bool("transparent_bg", False):
        children.append(
            render.Box(
                width = layout.width,
    ...
    # Add second bob (color changes over time) - only if show_joints is enabled
    if show_joints:
        children.append(
            render.Padding(
                pad = (x2 - 1, y2 - 1, 0, 0),
...
def generate_fade_out_frames(simulation, is_api_mode, config, layout, hsv_to_rgb, sim_idx, api_name):
    ...
    # Calculate number of fade frames (4-5 seconds at current delay)
    delay = 16 if config.get("speed", "slow") == "fast" else 33
    fade_duration_ms = 5000  # 5 seconds for longer, more visible fade
    ...
    return schema.Schema(
        ...
        fields = [
            ...
            schema.Toggle(
                ...
                desc = "Render only the last frame (for static image generation)",
                icon = "image",
                default = False,
            ),
            schema.Toggle(
                ...
                desc = "Render with transparent background (for overlay effects)",
                icon = "square",
                default = False,
            ),
        ],
